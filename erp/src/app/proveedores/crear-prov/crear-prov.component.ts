...
export class CrearProvComponent implements OnInit {
  ...
  crearProv(){
    ...
    //Ahora se hace el post y dentro lo que se hace al recibir la respuesta la mismo

    this.proveedoresService.postProveedor(this.proveedor).subscribe((resp:any)=>{
      // console.log(resp)
      this.router.navigate(['/listado-proveedores']);
      // Si hay éxito lleva a lista-proveedores
      this.enviando = false;
    },(error:any)=>{
      console.log(error);
  ...
  guardarProv(){
    const guardarProv = {
      ...
      tlf:this.proveedorForm.get('tlf').value,
